...
describe("POST /employee", () => {
  describe("with correct accessToken", () => {
    ...
    describe("and with correct employee data", () => {
      beforeAll(async () => {
        result = await request(app)
          .post(`${baseRoute}employee`)
          .send(newEmployeeDetails)
      ...
      it("have explanation text", () => {
        expect(result.text).toEqual("added successfully");
      });
    });
    describe("and with incorrect employee data", () => {
      beforeAll(async () => {
        result = await request(app)
          .post(`${baseRoute}employee`)
          .send({ ...newEmployeeDetails, currency: null, name: 123 })
          .set("Cookie", [`accessToken=${token}`]);
      });
      ...
      it("have object with errors", () => {
        expect(result.text).toEqual(
          JSON.stringify({
            errors: [{ name: "Invalid value" }, { currency: "Invalid value" }],
          })
        );
      });
    });
